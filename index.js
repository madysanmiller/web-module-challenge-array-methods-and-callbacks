...
console.log('task 1e', finals2014[0]['World Cup Winner 2014'])
...
function getYears(array, cBgetFinals) {
    let years = cBgetFinals(array).map(function (item) {
        return item.Year;
    });
...
function getWinners(array, cBgetFinals) {
    const winners = cBgetFinals(array).map(function(items){
...
function getAverageGoals(cBgetFinals) {
    const Goalstotal= cBgetFinals.reduce(function(acc, item) {
      
        return acc + item["Away Team Goals"] + item["Home Team Goals"];
       }, 0)   
        return (Goalstotal / cBgetFinals.length).toFixed(2)
 }
